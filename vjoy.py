...
CONST_DLL_VJOY = r"C:\Users\Admin\Desktop\Steering\vJoyInterface.dll"
...
class vJoy(object):
    ...
    def generateJoystickPosition(self, 
        ...
        # ???         ???        ???
        wSlider = 16393, wDial = 16393, wWheel = 0,
        # ???         ???        ???
...
def look_left(x):
    vj.open()
    joystickPosition = vj.generateJoystickPosition(wAxisXRot = int(16393*(1-x)))
    vj.update(joystickPosition)
    vj.close()
...
def throttle():
    vj.open()
    joystickPosition = vj.generateJoystickPosition(wAxisZRot = 19000)
    vj.update(joystickPosition)
    vj.close()
